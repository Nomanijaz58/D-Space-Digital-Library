...
EXPOSE 8080 8000
# Provide strict minimums for Java to survive Render free tier (512MB)
# Balanced to prevent both GC Thrashing (too little heap) and OS OOM (too much native RAM)
ENV JAVA_OPTS="-XX:+UseSerialGC -Xmx220m -Xms220m -XX:MaxMetaspaceSize=140m -Xss256k -XX:MaxDirectMemorySize=20m -XX:ReservedCodeCacheSize=32m"

# Copy the entrypoint script so it is available
COPY dspace-entrypoint.sh /dspace-entrypoint.sh
RUN chmod +x /dspace-entrypoint.sh

# On startup, run DSpace Runnable JAR via the entrypoint script
ENTRYPOINT ["/dspace-entrypoint.sh", "sh", "-c", "java $JAVA_OPTS $DSPACE_OPTS -Dserver.address=0.0.0.0 -Dserver.port=${PORT:-8080} -jar webapps/server-boot.jar"]
